...
Extracted from `docs/IMPLEMENTATION_PLAN.md` — the Next.js workstream for the cross-repo review in
...
Validated against the current codebase on 2026-08-16: none of it has landed yet — everything below
is still open. Roughly in the order phases unblock each other; not a hard commitment.

Everything below inherits `CLAUDE.md`'s conventions: `@/*` imports only, strict TypeScript, no
...
Supersedes the old next-step "seed/register the demo accounts against the real backend" — that gap
is closed as a side effect of 2.1 below.
...
script that registers them for real.
...
happens to call the backend next — the failure is invisible until it isn't. This is the same gap as
the old next-step "decide on token refresh/session UX".
...
| Node version | `@types/node` is `^20` while this doc notes Node 20.15 blocked `jsdom`. Pinning CI and the Dockerfile to Node 22 LTS would let `jsdom` back in and matches `node:22-alpine` in the Dockerfile — worth aligning. |
...
living, not a one-time snapshot. The "Suggested next steps" section above was merged in from
`docs/IMPLEMENTATION_PLAN.md` on 2026-08-16; keep the gap IDs (`G1`…`G21`) so it stays aligned with
`clients-infra/docs/ARCHITECTURE_IMPROVEMENTS.md`, and fold each item's detail back into
`README.md`/`CLAUDE.md` once it's how the app actually works, same as before.
